...
function readFile() {
    const path = './scripts/deployUpgradeableContract/result/5_serialized_tx.txt';

    const data = jetpack.read(path);
    return data;
}

function writeFile(hash: string) {
    const path = './scripts/deployUpgradeableContract/result/6_admin_proxy_contract_tx.json';
